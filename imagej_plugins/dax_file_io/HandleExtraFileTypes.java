...
public class HandleExtraFileTypes extends ImagePlus implements PlugIn {
	...
	private Object tryOpen(String directory, String name, String path) {
		...
		if ((file.isDirectory() && (lowerName.endsWith(".zarr") || new File(file, ".zarray").exists())) ||
			lowerName.equals(".zarray")) {
			return tryPlugIn("Zarr_Reader", path);
		}
        ...
        if (name.endsWith(".dax")) {
			return tryPlugIn("Dax_Reader", path);
		}
